...
class Precessing:
    ...
    def polarization_amplitude_and_phase(self, f):
        ...
        # define sin(2*psi) and cos(2*psi) robustly without tan division
        num_psi = (
        ...
        den2 = den_psi * den_psi + num_psi * num_psi
        # Avoid 0/0 by defaulting to cos(2psi)=1, sin(2psi)=0 when both zero
        safe = den2 != 0
        sin_2psi = np.zeros_like(den2, dtype=float)
        cos_2psi = np.ones_like(den2, dtype=float)
        sin_2psi[safe] = 2.0 * num_psi[safe] * den_psi[safe] / den2[safe]
        cos_2psi[safe] = (
            den_psi[safe] * den_psi[safe] - num_psi[safe] * num_psi[safe]
        ) / den2[safe]

        # define 2*psi + alpha using sum formulas
        sin_2pa = sin_2psi * cos_alpha + cos_2psi * sin_alpha
        cos_2pa = cos_2psi * cos_alpha - sin_2psi * sin_alpha
