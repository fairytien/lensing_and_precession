...
- Use the canonical `LBL_*` label constants defined in [`modules/plot_utils.py`](modules/plot_utils.py) instead of retyping lengthy LaTeX strings. They are the Python analogues of the `\newcommand` shortcuts in [`paper_lens_prec/preamble.tex`](../../paper_lens_prec/preamble.tex) and encode the correct typography (`\epsilon`, `\mathrm`, no `\mathit` on `h`).
