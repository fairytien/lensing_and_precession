...
- Use the canonical label constants defined in [`modules/plot_utils.py`](modules/plot_utils.py) instead of retyping lengthy LaTeX strings. Import what you need: `LBL_TD`, `LBL_MCZ`, `LBL_I`, `LBL_OMEGA`, `LBL_THETA`, `LBL_F` for axis labels; `LBL_H_L`, `LBL_H_UL`, `LBL_H_NP`, `LBL_H_RP`, `LBL_H_P`, `LBL_H_S`, `LBL_H_T` for waveform symbols; `LBL_EPS_LP`, `LBL_EPS_LNP`, `LBL_EPS_ST`, `LBL_MIN_EPS_LP`, `LBL_MIN_EPS_ST` for mismatch labels. These are the Python analogues of the `\newcommand` shortcuts in [`paper_lens_prec/preamble.tex`](../../paper_lens_prec/preamble.tex) and encode the correct typography (`\epsilon`, `\mathrm`, no `\mathit` on `h`).
