"""Create contour plots of Lindblom criterion computed from source SNR."""
...
def main(
    ...
    """Plot Lindblom contour from HDF5 file computed using source SNR.

    Args:
        results_dir: Directory containing the Lindblom HDF5 file.
        ...
        zero_contour: If True, draw a contour line at zero.
        vmin: Minimum value for color scale (default -0.05).
    """
    ...
    summary_path = os.path.join(
        results_dir,
        "best_match",
        f"lindblom_from_source_snr_I{_format_min_precision(I)}_mcz{_format_min_precision(mcz_min)}-{_format_min_precision(mcz_max)}Msun_td{_format_min_precision(td_min_ms)}-{_format_min_precision(td_max_ms)}ms_{orientation_tag}.h5",
    )

    if not os.path.isfile(summary_path):
        raise FileNotFoundError(
            f"Lindblom file not found: {summary_path}\n"
            f"Please run scripts/compute_lindblom_from_source_snr.py first."
        )

    logging.info(f"Loading Lindblom data from: {summary_path}")
    ...
    with h5py.File(summary_path, "r") as h5:
        ...
        required_datasets = ["mcz", "td", "lindblom_from_source_snr"]
        ...
        Lmap = np.array(h5["lindblom_from_source_snr"])
    ...
    # Set vmin to saturate values below threshold
    vmax = np.nanmax(Lmap)
    ...
    # Generate filename
    base_name = f"lindblom_from_source_snr_contour_I{_format_min_precision(I_value)}_mcz{_format_min_precision(mcz_min)}-{_format_min_precision(mcz_max)}Msun_td{_format_min_precision(td_min_ms)}-{_format_min_precision(td_max_ms)}ms_{orientation_tag}"
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        help="Directory containing the Lindblom HDF5 file.",
    )
    p.add_argument(
        "--td_min_ms",
        type=float,
    ...
    p.add_argument(
        ...
        help="Colorbar extension (default: min).",
    )
    p.add_argument(
        "--no-zero-contour",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=float,
        default=-0.05,
        help="Minimum value for color scale (default: -0.05).",
    )
